...
$scope.travellers = [];
	...
	while(i<=adults){
		var entry={};
		$scope.type.push('Adult');
		$scope.counter.push(i);
		entry.firstName='';
		$scope.middleName.push('');
		entry.lastName='';
		entry.nationality='';
		entry.passportNum='';
		entry.dateOfBirth='';
        entry.passportExpiryDate=moment(new Date()).toDate().getTime();
        entry.email="";
	...
	while(i<=baby){
        ...
        entry.dateOfBirth='';
        entry.passportExpiryDate=moment(new Date()).toDate().getTime();
        entry.email="";		$scope.popup1.opened.push(false);
		i++;
		$scope.travellers.push(entry);
